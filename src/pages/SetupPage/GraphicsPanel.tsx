...
export function GraphicsPanel() {
  ...
  function handleAdd() {
    ...
    if (!isValidGraphicUrl(newUrl.trim())) { setAddUrlError('Must be a valid http:// or https:// URL'); return }
  ...
  function handleEdit() {
    ...
    if (!isValidGraphicUrl(editTarget.url.trim())) { setEditUrlError('Must be a valid http:// or https:// URL'); return }
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono px-1.5 py-0.5 rounded bg-[--color-surface-raised] text-[--color-text-muted] uppercase">DSK</span>
                </div>
        ...
        {graphics.length === 0 &&!isLoading && (
          <p className="text-sm text-[--color-text-muted] py-4 text-center">No graphics yet. Add one to use as a DSK overlay.</p>
        )}
      </div>
        ...
        <Modal open title="Delete Graphic" onClose={() => setDeleteTargetId(null)} className="max-w-sm">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button variant="danger" onClick={() => { void removeGraphic(deleteTarget.id); setDeleteTargetId(null) }}>Delete</Button>
            </div>
          </div>
        </Modal>
      )}
      ...
      <Modal open={addOpen} title="New Graphic" onClose={() => { setAddOpen(false); setAddUrlError(null) }}>
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Lower Third" className="w-full px-3 py-2 rounded bg-[--color-surface-raised] border-[--color-border-strong] text-sm text-[--color-text-primary] focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="text-xs text-[--color-text-muted] uppercase tracking-wider block mb-1">URL</label>
            <input type="text" value={newUrl} onChange={(e) => { setNewUrl(e.target.value); setAddUrlError(null) }} placeholder="https://example.com/overlay" className="w-full px-3 py-2 rounded bg-[--color-surface-raised] border-[--color-border-strong] text-sm text-[--color-text-primary] focus:outline-none focus:border-orange-500" />
            {addUrlError && <p className="text-xs text-red-400 mt-1">{addUrlError}</p>}
          </div>
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="ghost" onClick={() => setAddOpen(false)}>Cancel</Button>
            <Button variant="active" onClick={handleAdd} disabled={!newName.trim() ||!newUrl.trim()}>Save</Button>
          </div>
        </div>
      </Modal>
      ...
      {editTarget && (
        <Modal open title="Edit Graphic" onClose={() => { setEditTarget(null); setEditUrlError(null) }}>
          <div className="flex flex-col gap-3">
            ...
            <div>
              <label className="text-xs text-[--color-text-muted] uppercase tracking-wider block mb-1">URL</label>
              <input type="text" value={editTarget.url} onChange={(e) => { setEditTarget({...editTarget, url: e.target.value }); setEditUrlError(null) }} className="w-full px-3 py-2 rounded bg-[--color-surface-raised] border-[--color-border-strong] text-sm text-[--color-text-primary] focus:outline-none focus:border-orange-500" />
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button variant="active" onClick={handleEdit} disabled={!editTarget.name.trim() ||!editTarget.url.trim()}>Save</Button>
            </div>
          </div>
        </Modal>
      )}
