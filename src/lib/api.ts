...
/** Path-segment encode for all resource IDs (closes #53). */
...
export const productionsApi = {
  ...
  get: (id: string) =>
    request<RawProduction>(`/api/v1/productions/${enc(id)}`)
  ...
  activate: (id: string) =>
    request<RawProduction>(`/api/v1/productions/${enc(id)}/activate`, { method: 'POST' })
      .then(normalizeProduction),

  deactivate: (id: string) =>
    request<RawProduction>(`/api/v1/productions/${enc(id)}/deactivate`, { method: 'POST' })
      .then(normalizeProduction),

  remove: (id: string) =>
    request<void>(`/api/v1/productions/${enc(id)}`, { method: 'DELETE' }),

  assignSource: (id: string, body: ProductionSourceAssignment) =>
    request<ProductionSourceAssignment & { _rev: string }>(`/api/v1/productions/${enc(id)}/sources`, {
      method: 'POST',
      body: JSON.stringify(body),
    }),

  unassignSource: (id: string, mixerInput: string) =>
    request<void>(`/api/v1/productions/${enc(id)}/sources/${enc(mixerInput)}`, { method: 'DELETE', silentStatuses: [404] }),

  assignGraphic: (id: string, body: ProductionGraphicAssignment) =>
    request<ProductionGraphicAssignment & { _rev: string }>(`/api/v1/productions/${enc(id)}/graphics`, {
      method: 'POST',
      body: JSON.stringify(body),
    }),

  unassignGraphic: (id: string, dskInput: string) =>
    request<void>(`/api/v1/productions/${enc(id)}/graphics/${enc(dskInput)}`, { method: 'DELETE' }),

  assignOutput: (id: string, outputId: string) =>
    request<ProductionOutputAssignment & { _rev: string }>(`/api/v1/productions/${enc(id)}/outputs`, {
      method: 'POST',
      body: JSON.stringify({ outputId }),
    }),

  unassignOutput: (id: string, outputId: string) =>
    request<void>(`/api/v1/productions/${enc(id)}/outputs/${enc(outputId)}`, { method: 'DELETE' }),
}
...
export const sourcesApi = {
  ...
  update: (id: string, body: Partial<Omit<ApiSource, 'id'>>) =>
    request<ApiSource>(`/api/v1/sources/${enc(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  remove: (id: string) =>
    request<void>(`/api/v1/sources/${enc(id)}`, { method: 'DELETE' }),
}
...
export const macrosApi = {
  list: (productionId: string) =>
    request<ApiMacro[]>(`/api/v1/productions/${enc(productionId)}/macros`),
  ...
  update: (productionId: string, macroId: string, body: Partial<Omit<ApiMacro, 'id'>>) =>
    request<ApiMacro>(`/api/v1/productions/${enc(productionId)}/macros/${enc(macroId)}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  remove: (productionId: string, macroId: string) =>
    request<void>(`/api/v1/productions/${enc(productionId)}/macros/${enc(macroId)}`, { method: 'DELETE' }),
}

export const audioApi = {
  discoverElements: (productionId: string) =>
    request<ApiAudioElement[]>(`/api/v1/productions/${enc(productionId)}/audio`),

  getElement: (productionId: string, elementId: string) =>
    request<{ element_id: string; properties: Record<string, unknown> }>(
      `/api/v1/productions/${enc(productionId)}/audio/${enc(elementId)}`,
    ),
...
export const productionConfigsApi = {
  ...
  update: (id: string, body: { name?: string; values?: Record<string, string | number | boolean> }) =>
    request<ProductionConfig>(`/api/v1/production-configs/${enc(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  remove: (id: string) =>
    request<void>(`/api/v1/production-configs/${enc(id)}`, { method: 'DELETE' }),
}
...
export const graphicsApi = {
  ...
  update: (id: string, body: { name?: string; url?: string }) =>
    request<ApiGraphic>(`/api/v1/graphics/${enc(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  remove: (id: string) =>
    request<void>(`/api/v1/graphics/${enc(id)}`, { method: 'DELETE' }),
}
...
export const outputsApi = {
  ...
  update: (id: string, body: { name?: string; url?: string }) =>
    request<ApiOutput>(`/api/v1/outputs/${enc(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  remove: (id: string) =>
    request<void>(`/api/v1/outputs/${enc(id)}`, { method: 'DELETE' }),
}
